...
public class ZipUtils {  
	 ...
	 ** @throws ZipException 压缩文件格式有误时抛出   
	 ** @throws IOException 当解压缩过程出错时抛出   */  
	public static ArrayList<String> getEntriesNames(File zipFile,String filepostfix) throws ZipException, IOException {   
		ArrayList<String> entryNames = new ArrayList<String>();    
		Enumeration<?> entries = getEntriesEnumeration(zipFile);   
		while (entries.hasMoreElements()) {     
			ZipEntry entry = ((ZipEntry)entries.nextElement());      
			String name=new String(getEntryName(entry).getBytes("GB2312"), "8859_1");
			if(name.endsWith(filepostfix)){
				entryNames.add(name);  
			}
		}    
		return entryNames;  
	...
	private static void zipFile(File resFile, ZipOutputStream zipout, String rootpath)
		...
		if (resFile.isDirectory()) {     
			...
			int realLength;      while ((realLength = in.read(buffer)) != -1) {       
